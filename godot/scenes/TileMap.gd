...
var t_270 :=[Vector2i(1,0), Vector2i(0,1), Vector2i(1,2), Vector2i(1,1)]
...
#variables for set_cells function
const COLS : int = 10
const ROWS : int = 20
...
func can_rotate():
	...
	for i in piece_type[temp_rotation_index]:
		if not is_free(i + cur_pos):
			cr = false
	return cr
#looks at cell at pos and returns layer ID. if there is no cell there, returns -1
func is_free(pos):
...
#check rows to see if they are full
func check_rows():
	var row : int = ROWS
	while row > 0:
		var count = 0
